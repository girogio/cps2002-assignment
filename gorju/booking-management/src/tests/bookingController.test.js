...
describe("Testing the booking controller", () => {
    ...
    describe("Testing updateBookingById", () => {

        it("it should reject and throw appropriate error if id is not supplied", (done) => {
            bookingController.updateBookingById().catch((err) => {
                expect(err.code).to.be.eq(400)
                expect(err.data).to.be.equal("Booking id is required.")
                done()
            })
        })

        it("it should resolve and change user", (done) => {
            ...
            const request = {
                booker_id: "deadbeefdeadbeefdeadbeef",
                number_plate: "BAD123",
                paid: true
            }

            let findByIdStub = sandbox.stub(bookingModel, 'findById').resolves(inputBooking)

            bookingController.updateBookingById("63aeefae51927a76c00b366a", request).then((booking) => {
                expect(findByIdStub).to.have.been.called
                expect(booking.booker_id).to.be.eq("deadbeefdeadbeefdeadbeef")
                expect(booking.number_plate).to.be.eq("BAD123")
                expect(booking.paid).to.be.true
                done()
            })
        })

        it("it should reject if booking is not saved", (done) => {
            ...
            bookingController.updateBookingById("63aeefae51927a76c00b366a", request).catch((err) => {
                expect(findByIdStub).to.have.been.called
                done()
            })
        })
